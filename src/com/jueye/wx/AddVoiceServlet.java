       ...
       int charnum=AddServlet.getContentWordCount(usercontens);
        ...
        if(i==1){
        	...
        	if(dollar>3+charnum/60 || dollar==3+charnum/60){
            	DBManager.updateDollarByopenid(openid,dollar-3-charnum/60);
            	
            	boolean sendstatus=SMS.execute(userphone, usercontens,voicename);
            	if(sendstatus){
            	 //返回值给微信小程序
                Writer out = response.getWriter(); 
                ...
                if(DBManager.showuid("wxstar", uid).equals("sucess")){
            		int num=Integer.parseInt(DBManager.showstarssignsnum("wxstar", uid, "stars"))+3;
